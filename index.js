...
* AFFIS <dev
* FADHILGRPHY <partner
* MLSPACETOON <author/repository
* MYBOT TEAM <fork
* JAN NGAKU2 SC INI PUNYA LU.
* KALO MAU REMAKE BOLEH AJA, PAHAM-kan..?
* KASIH AUTHORNYA.
* JANGAN CUMA NUMPANG NAMA DOANG.
* dahla segitu aja nuhun.
*/
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6287714745440@s.whatsapp.net","6287714745450@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('⌬','red'),color(']','white'),color('SCAN QR CODE !','white'),color('Dont','red'),color('Forget','white'),color(' SUBSCRIBE stardustlrlr','green'))
})
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("EITS! JAGA UCAPAN DONG!! 😠")
                    ...
                    return reply( "Tolong Dijaga Omongan-Nya Min ☺")
                }
            }
        }
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('ᴋᴀʀᴇɴᴀ ᴋᴀᴍᴜ ᴀᴅᴀʟᴀʜ ᴀᴅᴍɪɴ ɢʀᴏᴜᴘ, ʙᴏᴛ ᴛɪᴅᴀᴋ ᴀᴋᴀɴ ᴍᴇɴɢ-ᴋɪᴄᴋ ᴋᴀᴍᴜ')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`*[ ! ] 𝗪𝗔𝗥𝗡𝗜𝗡𝗚 𝗟𝗜𝗡𝗞 𝗚𝗥𝗢𝗨𝗣 𝗗𝗘𝗧𝗘𝗖𝗧𝗢𝗥*\nᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇᯇ\nʟɪɴᴋ ɢʀᴏᴜᴘ ᴛᴇʀᴅᴇᴛᴇᴋꜱɪ ɪᴍ ꜱᴏʀʀʏ ᴛᴏ ᴍᴀᴀꜰ\n${sender.split("@")[0]} ʏᴏᴜ ᴋᴀᴍᴜ ᴀᴋᴀɴ ᴅɪ ᴋᴇʟᴜᴀʀᴋᴀɴ/ᴅɪᴋɪᴄᴋ ᴅᴀʀɪ ɢʀᴏᴜᴘ ᴅᴀʟᴀᴍ 5 ᴅᴇᴛɪᴋ`)
						setTimeout( () => {
...
break
			...
			case 'ping':
        		if (!isRegistered) return reply(ind.noregis())
           	 await client.sendMessage(from, `Test speed..\nSpeed: ${processTime(time, moment())} _Second_`)
				break
                case 'help': 
				case 'menu':
				if (!isRegistered) return reply(ind.noregis())
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮??𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘀𝗶𝗺𝗶 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻??️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼??𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝗻𝘀𝗳𝘄 𝗱𝗶 𝗴𝗿𝗼???? 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
				break
			...
			if (!isPublic) return reply(mess.only.publikG)
					...
					teks += '\n'
					for (let mem of groupMembers) {
						teks += `╠ጸ @${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					mentions('╔══🜲〘 OTAG ALL 〙🜲══'+teks+'╚═〘 *MLSPACETOON BOT* 〙', members_id, true)
					break
				case 'delete':
				case 'del':
				case 'd':
